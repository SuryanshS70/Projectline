...
export const Route = createFileRoute("/_app/vendor/notifications")({
  head: () => ({
    meta: [
...
const groupLabels: Record<NotificationKind, string> = {
  ...
  project_update: "Client comments",
  feedback: "Client feedback",
};

function VendorNotifications() {
  const [items, setItems] = useState<NotificationRecord[]>(() => notificationsForRole("vendor"));
  const groups = items.reduce<Record<string, NotificationRecord[]>>((acc, n) => {
    (acc[n.kind] ||= []).push(n);
    return acc;
  }, {});
